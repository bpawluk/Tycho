...
public class InboxProcessorJobTests
{
    ...
    public async Task ExecuteAsync_WithTransactionalHandler_HandlesAndAcknowledgesInsideTransaction()
    {
        ...
        _transactionalHandlerMock.Verify(handler => handler.HandleAsync(It.IsAny<EventContext<TestEvent>>(), cancellationToken), Times.Once);
        _inboxConsumerMock.Verify(inbox => inbox.MarkAsHandledAsync(inboxEvent.ClaimId, cancellationToken), Times.Once);
        _inboxConsumerMock.Verify(inbox => inbox.MarkAsFailedAsync(inboxEvent.ClaimId, cancellationToken), Times.Never);
    }
    ...
    public async Task ExecuteAsync_WhenTransactionalProcessingFails_DisposesScopeBeforeFailureBookkeeping()
    {
        ...
        transaction
            .Setup(value => value.ExecuteAsync(It.IsAny<Func<CancellationToken, Task>>(), cancellationToken))
            .Returns((Func<CancellationToken, Task> operation, CancellationToken token) => operation(token));
        ...
        // Assert
        Assert.Equal(["inbox-1-created", "inbox-1-disposed", "inbox-2-created", "claim-failed", "inbox-2-disposed"], calls);
    }
    ...
    private void VerifyTransactionExecution(Times times, CancellationToken cancellationToken)
    {
