...
internal class UserStatusChangedEventHandler(UsersDbContext dbContext, IAuditModule auditModule) : ITransactionalEventHandler<UserStatusChangedEvent>
{
    public async Task HandleAsync(EventContext<UserStatusChangedEvent> context, CancellationToken cancellationToken)
    {
        ...
        await auditModule.ExecuteAsync(new RecordAuditRequest(context.Id, "User", user.Id, user.Status.ToString()), cancellationToken);
    }
