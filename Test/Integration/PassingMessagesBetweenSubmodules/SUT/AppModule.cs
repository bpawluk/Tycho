...
internal class AppModule : TychoModule
{
    protected override void DeclareIncomingMessages(IInboxDefinition module, IServiceProvider services)
    {
        ...
        module.Executes<BeginCommandWorkflowCommand>(commandData => alphaSubmodule.ExecuteCommand(commandData));
        module.Executes<BeginQueryWorkflowCommand>(commandData => alphaSubmodule.ExecuteCommand(commandData));
    }
    ...
    protected override void IncludeSubmodules(ISubstructureDefinition module, IServiceProvider services)
    {
        module.AddSubmodule<AlphaModule>(consumer =>
        {
            ...
            consumer.HandleCommand<AlphaCommand, AlphaBetaProxyHandler>();
            consumer.HandleQuery<AlphaQuery, string, AlphaBetaProxyHandler>();
        });

        module.AddSubmodule<BetaModule>(consumer =>
        {
            consumer.HandleEvent<BetaEvent, BetaGammaProxyHandler>();
            consumer.HandleCommand<BetaCommand, BetaGammaProxyHandler>();
            consumer.HandleQuery<BetaQuery, string, BetaGammaProxyHandler>();
        });

        module.AddSubmodule<GammaModule>((consumer =>
        {
            var thisModule = services.GetService<IModule>()!;
            consumer.HandleEvent<GammaEvent>(eventData => thisModule.PublishEvent<EventWorkflowCompletedEvent>(new(eventData.Id)));
            consumer.HandleCommand<GammaCommand>(commandData => thisModule.PublishEvent<CommandWorkflowCompletedEvent>(new(commandData.Id)));
